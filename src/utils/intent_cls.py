...
# Type alias for feature data (numpy arrays or torch tensors)
...
def classify_intent_from_cached_trajectory(
    ...
    """
    Classify intent from cached trajectory data.

    This function is specifically designed to work with cached trajectory data
    from NuPlan's Trajectory feature class. It converts the SE2 trajectory format
    (x, y, heading) to the format expected by classify_intent_from_trajectory().

    Args:
        trajectory_data: Cached trajectory data of shape [T, 3] with (x, y, heading)
        time_horizon: Time window for intent classification (seconds)
        sample_interval: Time interval between trajectory steps (seconds)
    ...
    Raises:
        ValueError: If trajectory data has invalid shape or values
        TypeError: If trajectory_data is not a valid array type

    Example:
        # For cached trajectory from EgoTrajectoryTargetBuilder
        cached_trajectory = trajectory_feature.data  # Shape: [T, 3] (x, y, heading)
        lateral_idx, longitudinal_idx = classify_intent_from_cached_trajectory(
            cached_trajectory, time_horizon=2.0, sample_interval=0.1
        )
    """
    # Input validation and error handling
    try:
        # Validate input type
        if not isinstance(trajectory_data, (np.ndarray, torch.Tensor)):
            raise TypeError(f"trajectory_data must be numpy array or torch tensor, got {type(trajectory_data)}")
        ...
        if trajectory_data.ndim != 2:
            raise ValueError(f"Trajectory data must be 2D, got {trajectory_data.ndim}D")

        if trajectory_data.shape[-1] != 3:
            raise ValueError(f"Trajectory data must have 3 channels (x, y, heading), got {trajectory_data.shape[-1]}")

        if len(trajectory_data) < 2:
            raise ValueError(f"Trajectory must have at least 2 time steps, got {len(trajectory_data)}")

        if np.isnan(trajectory_data).any() or np.isinf(trajectory_data).any():
        ...
        # Convert trajectory format from (x, y, heading) to (x, y, cos(heading), sin(heading))
        # for compatibility with classify_intent_from_trajectory
        num_steps = trajectory_data.shape[-2]
        x_coords = trajectory_data[..., 0]  # [T]
        y_coords = trajectory_data[..., 1]  # [T]
        heading_angles = trajectory_data[..., 2]  # [T]

        # # Validate heading values are in reasonable range
        # angle_magnitude = np.abs(heading_angles).max()
        # if angle_magnitude > 2 * np.pi:
        #     logger.warning(f"Heading angles seem to be in degrees rather than radians (max: {angle_magnitude})")
        # if angle_magnitude > 10 * np.pi:
        #     raise ValueError(f"Heading angles appear to be in invalid units (max: {angle_magnitude})")

        # Convert heading to cos/sin representation
        cos_heading = np.cos(heading_angles)
        sin_heading = np.sin(heading_angles)

        # Create trajectory tensor with shape [T, 4] for classify_intent_from_trajectory
        trajectory_4d = np.stack([x_coords, y_coords, cos_heading, sin_heading], axis=-1)

        # Apply existing classification function with error handling
        try:
            return classify_intent_from_trajectory(
                trajectory=trajectory_4d,
                dt=sample_interval,
                time_horizon=time_horizon,
                config=config
            )
        except Exception as e:
            logger.error(f"Error in classify_intent_from_trajectory: {e}")
            logger.info(f"Trajectory shape: {trajectory_data.shape}, dtype: {trajectory_data.dtype}")
            logger.info(f"Sample interval: {sample_interval}, time horizon: {time_horizon}")

            # Provide graceful fallback with default "stay in lane / maintain speed"
            lateral_default = LATERAL_INTENT_CLASSES["stay_in_lane"]
            longitudinal_default = LONGITUDINAL_INTENT_CLASSES["maintain_speed"]
            return lateral_default, longitudinal_default

    except (ValueError, TypeError) as e:
        logger.warning(f"Validation error in classify_intent_from_cached_trajectory: {e}")
        logger.info(f"Input type: {type(trajectory_data)}, shape: {trajectory_data.shape if hasattr(trajectory_data, 'shape') else 'unknown'}")
        ...
        logger.error(f"Unexpected error in classify_intent_from_cached_trajectory: {e}")
...
def create_intent_labels_from_cached_trajectory(
    trajectory_feature_data: FeatureDataType,
    lateral_intent: int = None,
    longitudinal_intent: int = None,
    time_horizon: float = 2.0,
    sample_interval: float = 0.1,
    config: IntentClassificationConfig = None
) -> Tuple[int, int]:
    """
    Create intent labels from cached trajectory feature data.

    This is a high-level convenience function that handles both the intent classification
    and label creation from cached trajectory data.

    Args:
        trajectory_feature_data: Trajectory feature data from cached EgoTrajectoryTargetBuilder
        lateral_intent: Optional pre-computed lateral intent index (if None, will compute)
        longitudinal_intent: Optional pre-computed longitudinal intent index (if None, will compute)
        time_horizon: Time window for intent classification (seconds)
        sample_interval: Time interval between trajectory steps (seconds)
        config: Configuration for intent classification thresholds
    ...
    Usage:
        # In target builder during model input preparation (not caching phase)
        trajectory_features = cached_data['trajectory']  # From EgoTrajectoryTargetBuilder
        lateral_idx, longitudinal_idx = create_intent_labels_from_cached_trajectory(
            trajectory_feature_data=trajectory_features.data
        )
    """
    # Convert to numpy if torch tensor
    if hasattr(trajectory_feature_data, 'numpy'):
        trajectory_np = trajectory_feature_data.numpy()
    elif hasattr(trajectory_feature_data, 'detach'):
        trajectory_np = trajectory_feature_data.detach().numpy()
    else:
        trajectory_np = np.asarray(trajectory_feature_data)

    # Compute intents if not provided
    if lateral_intent is None or longitudinal_intent is None:
        lateral_idx, longitudinal_idx = classify_intent_from_cached_trajectory(
            trajectory_data=trajectory_np,
            time_horizon=time_horizon,
            sample_interval=sample_interval,
            config=config
        )
    else:
        lateral_idx = lateral_intent
        longitudinal_idx = longitudinal_intent
